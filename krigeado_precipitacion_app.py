...
def kriging(filename):
    #Comprobación generando la tabla directamente con las estaciones
    df_estaciones = pd.read_csv(input+filename[0], sep=";")
    print(df_estaciones)
    ...
    region_path = r".\AppInput\assets\Contorno_Murcia\Murcia.shp" #'./ruta/al/archivo/Murcia.shp'
    ...
    for field in df_estaciones.keys()[6:]:
        ...
        hillshade_path = r".\AppInput\assets\my_hillshade.tif"  # Reemplaza con la ruta correcta
        ...
        shapefile_path = r"./AppInput/assets/Estaciones_Murcia/Estaciones.shp"  # Reemplaza con la ruta de tu archivo .shp
        ...
        if len(field) == 2:
            plt.savefig(output+"Dia0"+field[-1]+".png")
        else:
            plt.savefig(output+"Dia"+field[-2:]+".png")
        plt.close()
        ...
        plt.legend(loc='upper left')
        if len(field) == 2:
            plt.savefig(output+"AcumuladaDia0"+field[-1]+".png")
        else:
            plt.savefig(output+"AcumuladaDia"+field[-2:]+".png")
        plt.close()


    df = pd.DataFrame(results)
    print(df.head())
    df.to_csv(output+"ResultadosDiarios_KrigingPreciptiacion.csv", sep=';')
