...
values_murcia = np.asarray(estaciones_en_murcia["P19"])

# Definir la cuadrícula sobre la que realizarás el kriging
grid_lon, grid_lat = np.meshgrid(np.linspace(min(lons_murcia)*0.95, max(lons_murcia)*1.05, 100), 
                                 np.linspace(min(lats_murcia)*0.99, max(lats_murcia)*1.01, 100))
...
murcia_geometry = murcia_geometry.buffer(1000)
...
plt.contourf(grid_lon, grid_lat, masked_grid_z, cmap=daily_precipitation_cmap, norm=precipitation_norm)
plt.scatter(lons_murcia, lats_murcia, c=values_murcia, cmap=daily_precipitation_cmap, norm=precipitation_norm, edgecolor='k', s=100, label='Datos Observados')
#plt.colorbar(label='Precipitación')
cbar = plt.colorbar(ticks = daily_levels)
cbar.ax.set_yticklabels(daily_ticks) 
plt.title('Kriging de Precipitación en la Región de Murcia')
plt.xlabel('Longitud')
plt.ylabel('Latitud')
region.boundary.plot(color='black', linewidth=1.5, ax=plt.gca(), label='Contorno de Murcia')
plt.legend()
plt.show()
